...
function fresh(){return {version:VERSION,sessionId:`S-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,started:false,phase:'LOBBY',turn:0,round:1,teams:[],vote:null,lastResult:null,pitch:null,winnerId:null,usedQuestionIds:[],usedQuestionKeys:[],revealedCells:[1],answerSlots:{regular:0,battle:0,'pitch-final':0},log:[]}}
let G=fresh();
...
function gateTarget(target,xp){if(target>=30&&xp<16)return 29;if(target>=25&&xp<12)return 24;if(target>=17&&xp<8)return 16;if(target>=9&&xp<4)return 8;return target}
...
function publicState(){return {version:G.version,sessionId:G.sessionId,started:G.started,phase:G.phase,turn:G.turn,round:G.round,teams:G.teams.map(visibleTeam),vote:G.vote?publicVote():null,lastResult:G.lastResult,pitch:G.pitch,winnerId:G.winnerId,revealedCells:[...(G.revealedCells||[1])]}}
...
function shuffleQuestion(phase,actorId,scope=phase){
 ...
 const correctPair=pairs.find(x=>x.correct),wrongPairs=pairs.filter(x=>!x.correct);
 for(let i=wrongPairs.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[wrongPairs[i],wrongPairs[j]]=[wrongPairs[j],wrongPairs[i]]}
 const slotKey=bank, desired=(G.answerSlots[slotKey]||0)%pairs.length;G.answerSlots[slotKey]=(G.answerSlots[slotKey]||0)+1;
 const arranged=[...wrongPairs];arranged.splice(desired,0,correctPair);
 return {id:raw.id,phase:raw.phase,text:raw.text,options:arranged.map(x=>x.text),correct:desired,why:raw.why};
}
...
function speedXp(ms){if(ms<=7000)return 3;if(ms<=14000)return 2;return 1}
function resultRows(v){const results={};for(const id of v.eligible){const c=v.responses[id],ms=v.responseTimes?.[id];results[id]={answered:Number.isInteger(c),choice:Number.isInteger(c)?c:null,correct:c===v.question.correct,responseMs:Number.isFinite(ms)?ms:null,xpEarned:c===v.question.correct?speedXp(Number.isFinite(ms)?ms:20000):0}}return results}
function applyStandardResult(payload){const actor=team(payload.actorId);if(!actor)return;if(payload.kind==='battle'){
 ...
 if(ar?.correct)actor.xp+=ar.xpEarned||1;if(br?.correct&&opp)opp.xp+=br.xpEarned||1;
 if(opp&&ar?.correct&&!br?.correct){opp.blocked=true;actor.blocked=false}
 else if(opp&&!ar?.correct&&br?.correct){actor.blocked=true;opp.blocked=false}
 else {actor.blocked=false;if(opp)opp.blocked=false}
 }else{for(const [id,r] of Object.entries(payload.results)){const t=team(id);if(t&&r.correct)t.xp+=r.xpEarned||1}}
}
...
function roll(){if(!G.started||G.phase!=='TURN'||G.vote?.open||G.winnerId)return false;const actor=G.teams[G.turn];if(!actor||actor.blocked)return false;const value=nextDice(),from=actor.pos;let target=Math.min(30,from+value);target=gateTarget(target,actor.xp);G.phase='DICE';log('dice',{teamId:actor.id,value,from,target});emitState();broadcast('dice:rolling',{teamId:actor.id,duration:DUR.dice});later(()=>{broadcast('dice:result',{teamId:actor.id,value,from,target,duration:DUR.diceResult});later(()=>{G.phase='MOVE';emitState();let p=from;const moveOne=()=>{if(p>=target){later(()=>land(actor),500);return}p++;actor.pos=p;actor.phase=phaseForPos(p);emitState();broadcast('move:step',{teamId:actor.id,pos:p,target});later(moveOne,DUR.move)};moveOne()},DUR.diceResult)},DUR.dice);return true}
...
async function api(req,res,url){if(req.method==='GET'&&url.pathname==='/api/state')return json(res,200,publicState());if(req.method==='GET'&&url.pathname==='/api/trace')return json(res,200,{version:VERSION,state:publicState(),log:G.log.slice(-300)});if(req.method==='GET'&&url.pathname==='/healthz')return json(res,200,{ok:true,version:VERSION,phase:G.phase,started:G.started,teams:G.teams.map(t=>({id:t.id,joined:!!t.controlId,pos:t.pos,xp:t.xp,blocked:t.blocked})),vote:G.vote?{id:G.vote.id,open:G.vote.open,eligible:G.vote.eligible,voters:Object.keys(G.vote.responses),endsAt:G.vote.endsAt}:null,lastResult:G.lastResult?{voteId:G.lastResult.voteId,kind:G.lastResult.kind}:null,pitch:G.pitch,winnerId:G.winnerId});if(req.method==='GET'&&url.pathname==='/events'){res.writeHead(200,{'Content-Type':'text/event-stream','Cache-Control':'no-store','Connection':'keep-alive'});res.write(': connected\n\n');streams.add(res);sse(res,'hello',{version:VERSION,state:publicState()});req.on('close',()=>streams.delete(res));return true}if(req.method==='GET'&&url.pathname==='/qr.svg'){const text=url.searchParams.get('text')||'';res.writeHead(200,{'Content-Type':'image/svg+xml','Cache-Control':'no-store'});res.end(qrSvg(text));return true}if(req.method!=='POST')return false;let d={};try{d=await body(req)}catch{return json(res,400,{ok:false,error:'JSON inválido'})}
 ...
 if(url.pathname==='/api/vote'){const v=G.vote,t=team(String(d.teamId||'')),cid=String(d.controlId||'');if(!v?.open)return json(res,409,{ok:false,error:'Não há votação ativa.'});if(!t||t.controlId!==cid)return json(res,403,{ok:false,error:'Controle não vinculado a esta equipe.'});if(String(d.voteId)!==v.id)return json(res,409,{ok:false,error:'Votação desatualizada.'});if(t.blocked)return json(res,403,{ok:false,error:'Equipe em Crunch não pode votar.'});if(!v.eligible.includes(t.id))return json(res,403,{ok:false,error:'Sua equipe não participa desta votação.'});const choice=Number(d.choice);if(!Number.isInteger(choice)||choice<0||choice>=v.question.options.length)return json(res,400,{ok:false,error:'Alternativa inválida.'});if(Object.prototype.hasOwnProperty.call(v.responses,t.id))return json(res,409,{ok:false,error:'Resposta já registrada. O primeiro clique é definitivo.'});v.responses[t.id]=choice;v.responseTimes[t.id]=Math.max(0,Date.now()-(v.startedAt||Date.now()));log('vote',{voteId:v.id,teamId:t.id,choice,responseMs:v.responseTimes[t.id]});broadcast('vote:progress',{voteId:v.id,voterIds:Object.keys(v.responses),count:Object.keys(v.responses).length,total:v.eligible.length});emitState();const allAnswered=v.eligible.every(id=>Object.prototype.hasOwnProperty.call(v.responses,id));if(allAnswered)later(()=>finishVote(),650);return json(res,200,{ok:true,choice,count:Object.keys(v.responses).length,total:v.eligible.length,locked:true,allAnswered})}
 return false}
