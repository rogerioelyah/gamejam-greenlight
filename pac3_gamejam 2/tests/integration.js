...
(async()=>{
 ...
 try{
  ...
  let x=await get('/professor.html');assert(x.r.status===200&&x.t.includes('4.3.0-classroom'),'professor page');console.log('PASS professor page');
  ...
  x=await get('/aluno.html');assert(x.r.status===200&&x.t.includes('questionText'),'student page');console.log('PASS student page');
  ...
  assert(x.t.includes('if(renderBlockedControl()||!vote?.open||choice!==null)return'),'blocked vote client guard');console.log('PASS blocked client cannot submit vote');
  ...
  r=await post('/api/vote',{voteId:v.id,teamId:'T1',controlId:'C1',choice:correct});assert(r.ok,'vote first');let r2=await post('/api/vote',{voteId:v.id,teamId:'T1',controlId:'C1',choice:wrong});assert(r2.status===409,'second vote rejected');r=await post('/api/vote',{voteId:v.id,teamId:'T2',controlId:'C2',choice:wrong});assert(r.ok,'vote2');
  st=await jsonGet('/api/state');assert(st.vote.responses.T1===correct&&st.vote.responses.T2===wrong,'first vote remains');console.log('PASS first click is final and cannot be changed');
  await waitFor(async()=>{const s=await jsonGet('/api/state');return s.phase==='TURN'&&s.turn===2?s:null},2000,'post battle/crunch');st=await jsonGet('/api/state');assert(st.teams[0].xp>=1&&st.teams[0].xp<=3,'winner speed XP');assert(st.teams[1].xp===0&&!st.teams[1].blocked,'loser crunch consumed next turn');console.log('PASS Battle speed scoring + loser-only Crunch + auto unlock');
  ...
  await post('/api/test/set',{turn:0,teamId:'T1',pos:29,xp:16,phase:'TURN',started:true});await post('/api/roll',{teamId:'T1',controlId:'C1'});
  for(let i=1;i<=5;i++){
  ...
  const hz=await jsonGet('/healthz');assert(hz.version==='4.3.0-classroom'&&hz.winnerId==='T1','health');console.log('PASS health/diagnostic state');
